...
#include "../op_kernel/reduce_custom_tiling_data.h"
#include "../op_kernel/reduce_custom_tiling_key.h"

namespace optiling {
    const uint32_t BUFFER_NUM = 2;
    const uint32_t WS_SYS_SIZE =  16U * 1024U* 1024U; // 16MB
    const uint64_t UB_DATA_NUMBER_32 = 6;
    struct ReduceCustomCompileInfo {};
    struct ReduceCustomShapeInfo {
        uint64_t inputNum{0};
    ...
    static ge::graphStatus GetShapeAttrsInfo(gert::TilingContext* context, uint64_t ubSize, ReduceCustomShapeInfo& info)
    {
        OP_CHECK_IF(
    ...
    static ge::graphStatus CalculateCoreBlockNums(int64_t coreNum, ReduceCustomShapeInfo& info)
    {
    ...
    // tiling 分发入口
    static ge::graphStatus ReduceCustomTilingFunc(gert::TilingContext* context)
    {
        // ReduceCustomTilingData tiling;
        ReduceCustomTilingData* tiling = context->GetTilingData<ReduceCustomTilingData>();
        ReduceCustomShapeInfo shapeInfo;
        OP_CHECK_NULL_WITH_CONTEXT(context, tiling);
        OP_CHECK_IF(
            memset_s(tiling, sizeof(ReduceCustomTilingData), 0, sizeof(ReduceCustomTilingData)) != EOK,
    ...
    static ge::graphStatus TilingParseForReduceCustom([[maybe_unused]] gert::TilingParseContext* context)
    {   
        return ge::GRAPH_SUCCESS;
    }

    // tiling注册入口.
    IMPL_OP_OPTILING(ReduceCustom).Tiling(ReduceCustomTilingFunc).TilingParse<ReduceCustomCompileInfo>(TilingParseForReduceCustom);
} // namespace optiling