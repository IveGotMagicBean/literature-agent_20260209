...
/*!
 * \file reduce_custom.h
 * \brief
 */
#ifndef _REDUCE_CUSTOM_H
#define _REDUCE_CUSTOM_H

#include "kernel_operator.h"
#include "kernel_tiling/kernel_tiling.h"
#include "reduce_custom_tiling_data.h"
#include "reduce_custom_tiling_key.h"

namespace NsReduceCustom {
...
class ReduceCustom {
public:
    __aicore__ inline ReduceCustom(){};

    __aicore__ inline void Init(GM_ADDR x, GM_ADDR z,GM_ADDR workspace,const ReduceCustomTilingData* tilingData);
    __aicore__ inline void Process();

private:
    
    __aicore__ inline void CopyIn(int32_t progress);
    __aicore__ inline void ReduceCustomAxes0();
    __aicore__ inline void ReduceCustomAxes1();
    __aicore__ inline void ReduceCustomAxesAll();
...
template <typename T>
__aicore__ inline void ReduceCustom<T>::Init(GM_ADDR x, GM_ADDR z,GM_ADDR workspace, const ReduceCustomTilingData* tilingData)
{
...
template <typename T>
__aicore__ inline void ReduceCustom<T>::CopyIn(int32_t progress)
{
...
template <typename T>
__aicore__ inline void ReduceCustom<T>::Process()
{
   if(this->axes == 0){
        ReduceCustomAxes0();
    }else if (this->axes == 1){
...
template <typename T>
__aicore__ inline void ReduceCustom<T>::ReduceCustomAxes0(){
    AscendC::SyncAll();
...
template <typename T>
__aicore__ inline void ReduceCustom<T>::ReduceCustomAxes1(){
    AscendC::SyncAll();
    const uint32_t colNum = this->cols;
    const uint32_t rowNum = this->rows;
    const uint32_t totalElements = rowNum * colNum;
...
template <typename T>
__aicore__ inline void ReduceCustom<T>::ReduceCustomAxesAll(){
